...
function ContactSection() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/franchise-inquiry", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
...
function Chatbot() {
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: input, history: messages.slice(-8) }) });
      const data = await res.json();
      setMessages([...newMessages, { role: "assistant", content: data.response }]);
    } catch { setMessages([...newMessages, { role: "assistant", content: "Sorry, I'm having trouble connecting. Please try again!" }]); }
    setLoading(false);
  };
  return (
    <>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 40, justifyContent: "space-between", marginBottom: 40 }}>
          ...
          <div>
            ...
            <p style={{ fontFamily: "'Nunito', sans-serif", fontSize: 14, lineHeight: 2 }}>
              📧 maxburgerandmore@gmail.com<br />📍 Udaipur, Rajasthan, India<br />
